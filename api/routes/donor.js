...
router.post('/auth', async (req, res) => {
    ...
    const user = await Donor.findOne({email});//checking in Mongose model that enail is present
    if (user) {
...
router.post('/signup', async (req, res) => {
    ...
    // const degreeProgram = req.body.degreeProgram       FOR MID EVAL COMMENTED
    ...
    if(!req.body.email || !req.body.password){
        ...
        //Search 'Donor' table for a particular email and throw error if user already exists
        //else insert into 'Donor' table
  ...
  router.get('/:email', function (req, res) {
    ...
    Donor.findOne({email: req.params.email}, (err, vol) => {
        if (err)
...
router.post('/update',async function(req,res){
    ...
    const address = req.body.address
    const name = req.body.name
    await Donor.findOneAndUpdate({email:email},{address:address})
    await Donor.findOneAndUpdate({email:email},{name:name})
